...
# TO.solve_aula!(sol[4], sol_solvers[4])
# prob_lift = fetch(@spawnat 1 probs[])
# length(sol)
# size(sol[2])
...
# @btime sol, sol_solvers, xx = solve_admm($probs, $prob_load, $quad_params, $load_params, $true, $opts_al);
...
# pwd()
# idx = [(1:3)...,(7 .+ (1:3))...]
# output_traj(sol[2],idx,joinpath(pwd(),"trajectories/traj0.txt"))
# output_traj(sol[3],idx,joinpath(pwd(),"trajectories/traj1.txt"))
# output_traj(sol[4],idx,joinpath(pwd(),"trajectories/traj2.txt"))
# println(sol[2].x0[1:3])
# println(sol[3].x0[1:3])
# println(sol[4].x0[1:3])
