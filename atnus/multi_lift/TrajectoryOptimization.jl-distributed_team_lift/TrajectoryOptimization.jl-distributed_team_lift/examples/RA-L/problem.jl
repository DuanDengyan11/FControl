...
function gen_prob(agent, quad_params, load_params, r0_load=[0,0,0.25];
    ...
    # statically stable initial config 初始配平值 q四元数 u四旋翼四个桨的力和吊索力 uload吊挂物上的吊索力
    ...
    # Constants
    n_lift = 13 # number of state
    m_lift = 5  # number of manipulation
    n_load = 6
    m_load = num_lift

    # Calculated Params
    n_batch = num_lift*n_lift + n_load  # 状态量串联
    m_batch = num_lift*m_lift + m_load  # 操纵量串联
    α = asin(r_config/d)
    ...
    # Batch constraints
    r_inds = [(1:3) .+ i for i in (0:13:13*num_lift)]  # 1:3 14:16 27:29 40:42
    s_inds = 5:5:5*num_lift               # 5 10 15
    s_load = (1:num_lift) .+ 5*num_lift   #16:18
    function distance_constraint(c,x,u=zeros(m_batch))   #绳长
        r_load = x[r_inds[end]]
        for i = 1:num_lift
            r_lift = x[r_inds[i]]
    ...
    function force_constraint(c,x,u)   #吊索两边力相等
        u_load = u[s_load]
        for i = 1:num_lift
    ...
    quad_pairs = combinations(1:num_lift, 2)  #两个飞机避免碰撞
    ...
    function cI_cylinder(c,x,u)   #避免和障碍物碰撞
        c_shift = 1
        n_slack = 3
        for p = 1:length(_cyl)
            n_shift = 0
